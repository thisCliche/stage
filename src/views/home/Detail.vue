  ...
  <div class="detail">
		<Swiper v-if="imgLength">
			...
			<div class="info">
				...
				<ul class="tip">
					<li>
						Freight:${{details.v2ExpressPrice}}
					</li>
					<li>
						Delivery:{{details.v2ExpressTime}}
					</li>
					<li>
						Place:{{details.v2ExpressAddress}}
					</li>
				</ul>
				<ul class="list">
					<!-- <li>
					...
					<li>
						<span>Parameter</span><p>{{details.v2Type}}</p>
					</li>
					<li>
...
export default {
  ...
  data () {
		...
		collect() {
			if(this.collected){
				if(localStorage.getItem("token")===null){
					...
					let token = localStorage.getItem("token");
					this.baseJs.httpReq("/mall/user/goods/collection",this.id,{},"delete",token)
					.then(res => {
						// console.log(res);
						if(res.code === 20000){
							this.$toast({
				...
				if(localStorage.getItem("token")===null){
					...
					this.collected = true;
					let token = localStorage.getItem("token");
					this.baseJs.ajaxReq("/mall/user/goods/collection",{goodsId:this.id},"post",token)
					.then(res => {
						// console.log(res);
						if(res.code === 20000){
							this.$toast({
	...
	created() {
		...
		this.baseJs.httpReq("/mall/goods",this.id,{},"get",token)
		.then(res => {
			console.log(res);
			if(res.code === 20000){
